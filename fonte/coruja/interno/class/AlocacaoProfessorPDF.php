...
class AlocacaoProfessorPDF extends FPDF {
	...
	mysql_real_escape_string($_REQUEST['periodo']),mysql_real_escape_string($_REQUEST['espaco']),mysql_real_escape_string($_REQUEST['curso']));
...
public function obterGrade(){
	...
	ts.diaSemana,SUBSTRING(ts.horaInicio,1,5) as horaInicio,
	...
	mysql_real_escape_string($_REQUEST['periodo']),
        mysql_real_escape_string($_REQUEST['professor']),
 ...
 public function Cabecalho($largura,$curso,$professor,$periodoLetivo){
	...
	$txt='Espelho de Alocação do Professor : '.$professor;
